...
class R {
   ...
   */
  constructor(e) {
    this.cacheWillUpdate = async ({ response: t }) => this._cacheableResponse.isResponseCacheable(t) ? t : null, this._cacheableResponse = new ne(e);
  }
}
function q(s) {
  s.then(() => {
...
function ue(s) {
  return typeof s == "function" ? he(s) : (s instanceof IDBTransaction && ce(s), ae(s, ie()) ? new Proxy(s, M) : s);
}
function f(s) {
...
const N = (s) => P.get(s);
function de(s, e, { blocked: t, upgrade: n, blocking: a, terminated: r } = {}) {
...
const pe = ["get", "getKey", "getAll", "getAllKeys", "count"], me = ["put", "add", "delete", "clear"], O = /* @__PURE__ */ new Map();
function F(s, e) {
...
const ge = "workbox-expiration", b = "cache-entries", H = (s) => {
...
}, A = (s) => [d.prefix, s, d.suffix].filter((e) => e && e.length > 0).join("-"), ye = (s) => {
...
const z = "GET", E = (s) => s && typeof s == "object" ? s : { handle: s };
class w {
  /**
...
class Ce {
  ...
  handleRequest({ request: e, event: t }) {
    ...
    return l instanceof Promise && (this._catchHandler || p) && (l = l.catch(async (h) => {
      if (p)
        try {
          return await p.handle({ url: n, request: e, event: t, params: r });
        } catch (I) {
...
let C;
const xe = () => (C || (C = new Ce(), C.addFetchListener(), C.addCacheListener()), C);
function k(s, e, t) {
  let n;
  if (typeof s == "string") {
...
class Ie {
  ...
  async cachePut(e, t) {
    ...
    if (!r)
      return !1;
    const { cacheName: i, matchOptions: o } = this._strategy, c = await self.caches.open(i), l = this.hasCallback("cacheDidUpdate"), p = l ? await Ee(
    ...
    try {
      ...
      if (h instanceof Error)
        throw h.name === "QuotaExceededError" && await De(), h;
    }
...
class U {
  ...
  handleAll(e) {
    ...
    const t = e.event, n = typeof e.request == "string" ? new Request(e.request) : e.request, a = "params" in e ? e.params : void 0, r = new Ie(this, { event: t, request: n, params: a }), i = this._getResponse(r, n, t), o = this._awaitComplete(i, r, n, t);
    return [i, o];
  }
...
self.addEventListener("notificationclick", (s) => {
  ...
  const { badge: t, body: n, data: a, dir: r, icon: i, lang: o, tag: c, timestamp: l, title: p } = e, { access_token: h, notification_type: I } = a, Y = new Promise((Z) => {
    s.notification.close();
    const y = `/#/notifications?id=${c}&access_token=${btoa(h)}`;
    self.clients.matchAll({
      type: "window",
      includeUncontrolled: !0
    }).then((m) => {
      var v;
      if (console.log("NOTIFICATION CLICK clients 1", m), m.length && "navigate" in m[0]) {
        console.log("NOTIFICATION CLICK clients 2", m);
        const g = m.find(
        ...
        console.log("NOTIFICATION CLICK navigate", y), g ? (console.log("NOTIFICATION CLICK postMessage", g), (v = g.postMessage) == null || v.call(g, {
          ...
          id: c,
          accessToken: h
        }), g.focus()) : (console.log("NOTIFICATION CLICK openWindow", y), self.clients.openWindow(y));
      } else
        console.log("NOTIFICATION CLICK openWindow", y), self.clients.openWindow(y);
      Z();
    });
  });
  s.waitUntil(Y);
